...
def resume_checkpoint(
    ...
    if os.path.isfile(checkpoint_path):
        ...
        if isinstance(checkpoint, dict) and 'state_dict' in checkpoint:
            ...
            if dino_loss and 'dino_loss' in checkpoint['args']:
                if log_info:
                    _logger.info('Restoring DINO loss state from checkpoint...')
                dino_loss.center = vars(checkpoint['args'])['dino_loss'].center.cuda()

            if early_stopping and 'patience_counter' in checkpoint['args']:
                if log_info:
                    _logger.info('Setting counter patience of early stopping...')
                early_stopping.counter = vars(checkpoint['args'])['patience_counter']
...
def cosine_scheduler(base_value, final_value, epochs, niter_per_ep=1, warmup_epochs=0, start_warmup_value=0):
...
def extract_best_epochs_from_summary(summary: str, metric: str):
    with open(summary, 'r') as file:
        # Read the first line to get the column headers
        lines = file.readlines()

    header = lines[0].strip().split(',')
    column_index = header.index("eval_" + metric)
    values = [float(line.strip().split(',')[column_index]) for line in lines[1:]]
    best = max(values)
    return best
